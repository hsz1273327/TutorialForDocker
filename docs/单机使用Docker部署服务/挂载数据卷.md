...
1. CIFS面向网络连接的共享协议,对网络传输的可靠性要求高,常基于TCP/IP;NFS是独立于传输的,可使用TCP或UDP.
...
无论使用哪一种方式,我们都只能使用`volume`方式挂载数据卷

## 使用nfs作为数据卷

使用nfs做数据卷需要先在宿主机上安装`nfs-utils`(debian使用`sudo apt install -y nfs-common`)

v3: 'type=volume,source=nfsvolume,target=/app,volume-driver=local,volume-opt=type=nfs,volume-opt=device=:/var/docker-nfs,volume-opt=o=addr=10.0.0.10' 

v4: 'type=volume,source=nfsvolume,target=/app,volume-driver=local,volume-opt=type=nfs,volume-opt=device=:/var/docker-nfs,"volume-opt=o=addr=10.0.0.10,rw,nfsvers=4,async"'

## 使用cifs作为数据卷

使用cifs做数据卷需要先在宿主机上安装`cifs-utils`(debian使用`sudo apt install -y cifs-utils`)