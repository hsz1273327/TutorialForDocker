...
+ 了解yaml格式
+ bash/powershell命名行基础

## 关于本文的一些说明

本文会从始至终构造一个简单的http服务,然后演示各种环境下如何部署它让其可用.

为了体现文章的通用性,本文不会介绍与平台架构深度绑定的内容(主要是一些扩展),同时例子都是在`linux/arm64`平台测试执行的.因此可以放心使用.
...
这两者就要看不同的实现了.
物理机上直接对网卡hack可以达到几乎无损失的网络,其他网络方式除非是`--host`模式都会有不同程度的损失.也是看方案的,这块我们会在后文不同执行环境的网络部分详细说明并给出评测结果.
...
首先必须承认相比裸机,docker是需要额外的硬盘空间的,为了不影响宿主机的正常运行,我们最好将docker的目录迁移到非系统盘,一般常见的操作是使用链接.注意要将原有的数据迁移过去需要先停止docker的运行.我们以debian为例,将docker相关的数据迁移到`/data/docker`目录
...
2. layout网络
    ...
    layout网络是docker swarm默认下最常规的网络类型,会根据配置的不同损失大约10%至50%的io性能,具体多少要看配置情况,但是架不住功能强大.将其带来的自动服务注册服务发现和负载均衡在外部实现性能损失未必能优化到比它少,因此该用还是得用.
    如果实在嫌弃可以全部用host网络部署,这些功能全部放在外部实现.

3. log

    docker默认会记录所有打印出来的log,我也更加推荐容器中log直接输出到stdout,但这样log就会不可控.如果不做统一的log收集,比较推荐的方式是在执行容器时设置好log保存行为,我们使用`--log-opt max-size=10m`设置切割之前日志的最大大小,`--log-opt max-file=3`设置存在的最大日志文件数.
    这样如果切割日志会创建超过阈值的文件数,则会删除最旧的文件.

    但这并不解决log收集的问题,docker提供了多种log收集的driver,比较通用的方法是将log通过`fluent-bit`收集后输送到合适的log仓库落库,这样本地log基本就不存在了.这个会在后续文章中介绍.
...
如何发现这些漏洞呢?一个通用的方式是使用[harbor](https://github.com/goharbor/harbor)做为镜像仓库,利用其中自带的镜像扫描工具做安全性检查,这些会在后文中有相应介绍.

如何防范这些攻击呢?最简单的办法是更新操作系统到最新,一般漏洞都是操作系统带来的,通常操作系统的更新一个很重要的目的就是修复漏洞.
