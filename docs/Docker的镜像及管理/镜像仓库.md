...
一般生产环境下我们的docker都是部署在linux上这种情况下配置文件的位置一般在`/etc/docker/daemon.json`;

而如果是使用`docker desktop`那么在`docker desktop`的设置项中进入`Docker Engine`,左侧会有一个json形式的配置文件.这个就是docker的配置文件了.
...
# 你的harbor对外的域名,不要使用`localhost`,`0.0.0.0`或者`127.0.0.1`
