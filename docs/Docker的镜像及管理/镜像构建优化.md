...
+ [alpine](https://registry.hub.docker.com/_/alpine),`2m到3m`的大小.前文已经介绍过,这边不再复述,本质上可以理解为`busybox:musl`+`apk`.
    ...
    如果要在编译阶段使用`alpine`,而且我们依赖`glibc`,那么我们需要先安装`build-base`这个工具集.
...
+ 使用`alpine`
    ...
    RUN echo "deb https://mirrors.ustc.edu.cn/debian buster-updates main" >> /etc/apt/sources.list
    RUN apt update -y \
...
+ 使用`debian-slim`
    ...
    # 压缩可执行文件
    FROM --platform=$TARGETPLATFORM golang:1.15 as upx
    ARG TARGETARCH
    WORKDIR /code
    # 安装upx
    RUN echo " " > /etc/apt/sources.list 
    ...
    # 压缩可执行文件
    COPY --from=build_bin /code/hellodocker-go .
    RUN upx --best --lzma -o hellodocker-go hellodocker
